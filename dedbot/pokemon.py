...
class PokeCatcher():
    ...
    def is_wild_pokemon(message):
        ...
        if 'title' in embed.keys() and embed['title'].startswith("A wile po"):
            return True
    ...
    async def on_message(self, message):
        if self.message_matters(message) and is_wild_pokemon(message) and if pokemon in open('raremons.txt').read():
            if self.channels[message.channel]:
                await self.client.message.send(message.channel, 'p!catch ' + get_wild_pokemon(message))
